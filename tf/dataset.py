...
class Dataset():
    ...
    def _load_data(self, data_file):
        ...
        for line in open(data_file):
            ...
            if self.is_training and len(s1)!=0 and len(s2)!=0:
                y = int(line[-1])  # 1 or [1]
                yield s1, s2, y
            else:
    ...
    def process_data(self, data_file, sequence_length=20):
        ...
        """
        '''
        if data_file == self.data_file and os.path.exists(self.npy_file) and self.is_training:  # only for all train data
        ...
        for s1, s2, y in data_iter:
            ...
            if self.is_training:# and s1_len!=0 and s2_len!=0:
                #print [s1_pad[0], s2_pad[0], y, s1_len, s2_len]
...
if __name__ == '__main__':
    ...
    d_word = Dataset(char_level=False, embedding_dim=300)
